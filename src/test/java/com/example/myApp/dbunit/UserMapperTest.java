...
class UserServiceTest {
    ...
    void findByPk() {
        ...
        // id=3の検索結果
        User actual = userService.select(1);
        // 検証：期待値と一致していること
    ...
    @Test
    @DatabaseSetup("/testdata/UserServiceTest/init-data")
    @ExpectedDatabase(value = "/testdata/UserServiceTest/init-data", assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED) // テスト実行後に１件データが追加されていること
    void insert() {
        // 登録するデータを準備
        ...
        newUser.setImageUrl("gazou1");
         //登録実行
        int createdCount = userService.insert(newUser);
         //検証：1件の追加に成功していること
    ...
    void update() {
        ...
        // 更新実行
        int updatedCount = userService.update(updateUser);
        // 検証：1件の更新に成功していること
    ...
    void delete() {
        // id=1のレコードを削除
        int deletedCount = userService.delete(1);
        // 検証：1件の削除に成功していること
